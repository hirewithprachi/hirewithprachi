-- PDF Download System Migration (Targeted)
-- This adds only the new tables needed for the PDF download system
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'resources') THEN
...
BEGIN
  ...
  -- Add file_size_bytes if it doesn't exist
  ...
  -- Add requires_lead_capture if it doesn't exist
  ...
  -- Add preview_image_url if it doesn't exist
...
-- Enable RLS on resources table if it exists
DO $$
BEGIN
...
DROP POLICY IF EXISTS "Public read access to active categories" ON resource_categories;
CREATE POLICY "Public read access to active categories" ON resource_categories
  FOR SELECT USING (is_active = true);

-- Public read access to free resources
DROP POLICY IF EXISTS "Public read access to free resources" ON resources;
CREATE POLICY "Public read access to free resources" ON resources
  FOR SELECT USING (is_premium = false OR is_premium IS NULL);

-- Admin access policies (check if admin_users table exists)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'admin_users') THEN
    ...
    DROP POLICY IF EXISTS "Admin full access to categories" ON resource_categories;
    ...
    -- Admin access to downloads
    DROP POLICY IF EXISTS "Admin access to downloads" ON resource_downloads;
    ...
    -- Admin access to tokens
    DROP POLICY IF EXISTS "Admin access to tokens" ON download_tokens;
    ...
    CREATE POLICY "Admin full access to resources" ON resources
      FOR ALL USING (
        EXISTS (
          SELECT 1 FROM admin_users 
          WHERE user_id = auth.uid() 
...
EXCEPTION
  WHEN undefined_function THEN
    -- update_updated_at_column function doesn't exist, skip trigger
    NULL;
END $$;
