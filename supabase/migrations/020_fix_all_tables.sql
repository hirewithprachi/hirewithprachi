-- Fix All Tables Migration (CLI Only)
-- This ensures all website functionality works with Supabase

-- ========================================
-- 1. VIDEOS TABLE (for service pages)
-- ========================================
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'resources' AND column_name = 'mime_type') THEN
    ALTER TABLE public.resources ADD COLUMN mime_type TEXT DEFAULT 'application/pdf';
  END IF;
...
-- 4. CREATE INDEXES
-- ========================================
CREATE INDEX IF NOT EXISTS idx_videos_service_category ON videos(service_category);
...
CREATE POLICY "Public read active categories" ON resource_categories
  FOR SELECT USING (is_active = true);

DROP POLICY IF EXISTS "Admin full access categories" ON resource_categories;
CREATE POLICY "Admin full access categories" ON resource_categories
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM admin_users 
      WHERE user_id = auth.uid() 
...
-- Resources policies (update existing)
DROP POLICY IF EXISTS "Public read free resources" ON resources;
CREATE POLICY "Public read free resources" ON resources
  FOR SELECT USING (is_premium = false OR is_premium IS NULL);

DROP POLICY IF EXISTS "Admin full access resources" ON resources;
CREATE POLICY "Admin full access resources" ON resources
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM admin_users 
      WHERE user_id = auth.uid() 
...
DROP POLICY IF EXISTS "Users access own tokens" ON download_tokens;
CREATE POLICY "Users access own tokens" ON download_tokens
  FOR SELECT USING (user_email = auth.email());

DROP POLICY IF EXISTS "Admin access tokens" ON download_tokens;
CREATE POLICY "Admin access tokens" ON download_tokens
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM admin_users 
      WHERE user_id = auth.uid() 
...
DROP POLICY IF EXISTS "Admin access downloads" ON resource_downloads;
CREATE POLICY "Admin access downloads" ON resource_downloads
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM admin_users 
      WHERE user_id = auth.uid() 
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM resources WHERE title = 'Employee Handbook Template') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM resources WHERE title = 'POSH Compliance Checklist') THEN
    INSERT INTO resources (
      title, description, category_id, type, file_path, file_size_bytes, 
      mime_type, tags, is_featured, requires_lead_capture, ai_summary
    ) VALUES (
  ...
  IF NOT EXISTS (SELECT 1 FROM resources WHERE title = 'Job Description Template Pack') THEN
    INSERT INTO resources (
      title, description, category_id, type, file_path, file_size_bytes, 
      mime_type, tags, is_featured, requires_lead_capture, ai_summary
    ) VALUES (
