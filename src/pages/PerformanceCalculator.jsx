...
export default function PerformanceCalculator() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const filename = await downloadCalculatorPDF('performance', pdfData, leadData);
      ...
      const report = `Employee Performance Analysis Report
=====================================

Company: ${result.companyInfo.name}
Employee: ${result.formData.employeeName}
Department: ${result.formData.department}
Manager: ${result.companyInfo.manager}
...
DETAILED BREAKDOWN:
- Goal Achievement: ${result.breakdown.goals}/10
- Quality of Work: ${result.breakdown.quality}/10
- Productivity: ${result.breakdown.productivity}/10
- Teamwork: ${result.breakdown.teamwork}/10

RECOMMENDATIONS:
${result.score >= 90 ? 
  'Outstanding performance! This employee is exceeding expectations and should be considered for advancement opportunities.' :
  result.score >= 80 ?
  'Excellent performance. Continue providing growth opportunities and recognition for achievements.' :
  result.score >= 70 ?
  'Good performance. Focus on specific areas for improvement and provide additional support where needed.' :
  result.score >= 60 ?
  'Satisfactory performance. Develop a performance improvement plan with clear goals and regular check-ins.' :
  'Performance needs improvement. Create a detailed improvement plan with specific milestones and frequent feedback sessions.'
}
...
Professional HR Solutions & Consulting
    `;
      ...
      a.href = url;
      a.download = `performance-analysis-${result.formData.employeeName.toLowerCase().replace(/\s+/g, '-')}.txt`;
      a.click();
