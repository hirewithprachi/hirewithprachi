...
export default function ResumeParser() {
  ...
  const handleDownload = async () => {
    ...
    try {
      const filename = await downloadCalculatorPDF('resume-parser', result, companyInfo);
      setDownloaded(true);
...
Name: ${result.parsedData.candidate.name}
Email: ${result.parsedData.candidate.email}
Phone: ${result.parsedData.candidate.phone}
Location: ${result.parsedData.candidate.location}
Experience: ${result.parsedData.candidate.experience}
Education: ${result.parsedData.candidate.education}

SKILLS:
${result.parsedData.candidate.skills.join(', ')}

CERTIFICATIONS:
${result.parsedData.candidate.certifications.join(', ')}

ANALYSIS RESULTS:
- Match Score: ${result.parsedData.analysis.matchScore}%
- Experience Relevance: ${result.parsedData.analysis.experienceRelevance}%
- Skill Match: ${result.parsedData.analysis.skillMatch}%
- Education Fit: ${result.parsedData.analysis.educationFit}%
- Overall Rating: ${result.parsedData.analysis.overallRating}/100

RECOMMENDATIONS:
${result.parsedData.recommendations.map(r => `• ${r}`).join('\n')}

RED FLAGS:
${result.parsedData.redFlags.map(f => `• ${f}`).join('\n')}
...
AI-Powered Resume Analysis
    `;
      ...
      a.href = url;
      a.download = `resume-analysis-${result.jobTitle.toLowerCase().replace(/\s+/g, '-')}.txt`;
      a.click();
