...
export default function TurnoverCalculator() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const filename = await downloadCalculatorPDF('turnover', pdfData, leadData);
      ...
      const report = `Employee Turnover Analysis Report
=====================================
...
TURNOVER ANALYSIS:
- Total Employees: ${result.formData.totalEmployees}
- Employees Left: ${result.formData.employeesLeft}
- Time Period: ${result.formData.timePeriod} months
- Turnover Rate: ${result.turnoverRate}%
- Turnover Level: ${getTurnoverLevel(parseFloat(result.turnoverRate)).level}

COST ANALYSIS:
- Total Turnover Cost: ₹${result.totalCost}
- Annualized Cost: ₹${result.annualizedCost}

COST BREAKDOWN:
- Recruitment Costs: ₹${result.breakdown.recruitment}
- Training Costs: ₹${result.breakdown.training}
- Productivity Loss: ₹${result.breakdown.productivity}

RECOMMENDATIONS:
${parseFloat(result.turnoverRate) < 10 ? 
  'Excellent retention! Continue your current practices.' :
  parseFloat(result.turnoverRate) < 20 ?
  'Moderate turnover. Consider improving employee engagement and retention strategies.' :
  parseFloat(result.turnoverRate) < 30 ?
...
Professional HR Solutions & Consulting
    `;
      ...
      a.href = url;
      a.download = `turnover-analysis-${result.companyInfo.name.toLowerCase().replace(/\s+/g, '-')}.txt`;
      a.click();
